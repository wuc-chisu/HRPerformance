...
export async function POST(request: Request) {
  try {
    ...
    const normalizedJoinDate =
      typeof joinDate === "string" ? joinDate.trim() : "";
    ...
    if (existingEmployee) {
      ...
      if (sameName && sameEmail) {
        return NextResponse.json({
          ...
          professionalDevelopmentRecords: normalizeProfessionalDevelopmentRecords(
            (existingEmployee as any).professionalDevelopmentRecords
          ),
          onboarding: formatOnboarding(existingEmployee),
          weeklyRecords: existingEmployee.weeklyRecords.map((record) => ({
            startDate: formatDateForResponse(record.startDate),
            endDate: formatDateForResponse(record.endDate),
            plannedWorkHours: record.plannedWorkHours,
            actualWorkHours: record.actualWorkHours,
      ...
      return NextResponse.json(
        {
          ...
          details: `Employee ID ${normalizedEmployeeId} is already used by ${existingEmployee.name} (${existingEmployee.email}).`,
          employeeId: existingEmployee.employeeId,
          existingName: existingEmployee.name,
          existingEmail: existingEmployee.email,
          createdAt: formatDateTimeForResponse(existingEmployee.createdAt),
        },
        { status: 409 }
      );
    }
    ...
    const employee = await (prisma as any).employee.create({
      data: {
        ...
        onboardingStep2Forms: buildDefaultStep2Forms(
          parseDateForDatabase(normalizedJoinDate),
          false
        ),
