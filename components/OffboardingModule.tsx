...
function matchesITRole(employee: Employee) {
  const dept = employee.department || "";
  const position = employee.position || "";
  return /information technology|\bit\b|system admin|administrator/i.test(dept) || /information technology|\bit\b|system admin|administrator/i.test(position);
}

export default function OffboardingModule({ employees }: OffboardingModuleProps) {
  ...
  useEffect(() => {
    if (!itResponsible) {
      const firstIt = itCandidates[0]?.name || sortedEmployees[0]?.name || "";
  ...
  const firstItName = itCandidates[0]?.name || sortedEmployees[0]?.name || "";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3 mb-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-lg border border-slate-300 bg-slate-50"
              >
                {(itCandidates.length > 0 ? itCandidates : sortedEmployees).map((emp) => (
                  <option key={emp.id} value={emp.name}>{emp.name}</option>
                ))}
              </select>
            </div>
