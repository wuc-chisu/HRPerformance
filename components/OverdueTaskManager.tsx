...
interface OverdueTaskManagerProps {
  ...
  onUpdate: (details: OverdueTaskDetail[]) => Promise<void> | void;
  onClose: () => void;
  title?: string;
}
...
export default function OverdueTaskManager({
  ...
  const handleSave = async () => {
    try {
      ...
      console.error("Failed to save overdue task details:", error);
      alert(error instanceof Error ? error.message : "Failed to save overdue task details.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-linear-to-r from-red-300 to-red-400 px-6 py-4 sticky top-0">
